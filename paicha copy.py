...
if option=="UM":
    ...
    if st.button("One Click Send Email"):
        ...
        email_body_html = """<p>Hi Team,</p><br><p>We are writing to inform you that client’s trading behavior causing excessive server load.<br>Please inform clients to adjust EA configurations and trading behaviors to avoid causing server overloading.</p><br>"""

        # Build ST sections as HTML
        st_html = ""
        if st_pasted_text:
            st_html += "<p><u><b>ST</b></u><br>"
            st_html += "<br>".join(st_result_df[0]) + "</p><br>"

        if st2_pasted_text:
            st_html += "<p><u><b>ST2</b></u><br>"
            st_html += "<br>".join(st2_result_df[0]) + "</p><br>"

        if st4_pasted_text:
            st_html += "<p><u><b>ST4</b></u><br>"
            st_html += "<br>".join(st4_result_df[0]) + "</p><br>"

        # Append ST sections into email body
        email_body_html += st_html

        # Add remaining content
        email_body_html += """<p>What would CS Team need to do?</p><br><p>Please kindly inform client to adjust EA configuration or trading behavior accordingly and help to assist clients if they need. The threshold value was listed as below if client would like to know. System Admin would also preserve the right to take execution regarding to other abnormal behaviors causing server excessive loading. For instance, consistently login and logout without actual operations/several times of request sending to server within a second/unknown network pumping, etc.</p><br><p>More than 3 times within a second<br>More than 600 times within 30 minutes.</p><br><p>Best Regards,<br>System Admin</p>"""

        # print(email_body_html)
        # st.code(email_body_html, language="text")

        url = "http://localhost:5678/webhook/startrader"
